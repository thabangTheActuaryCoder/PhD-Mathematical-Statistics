...
\begin{document}
	...
	rate: two models can agree on every forward value while assigning different
	probabilities to large moves, and a caplet separates them. Importing a
	volatility parameter from the legacy market therefore yields a number without
	establishing that it follows uniquely from what was observed.
	...
	trading risk is the realised profit or loss from hedging the liability. A
	closely calibrated model can still produce large losses when trading
	constraints or changing dynamics bind, and small average hedging error says
	nothing about the upper tail.
	...
	structure is simplified, and that simplification is stated. The application is
	the JIBAR--ZARONIA transition; a retrospective study in a currency where
	successor options did trade offers a further validation route, subject to
	licensed and comparable data.
	...
	position in an illiquid one, making trading permissions part of the
	mathematical problem, and \citet{feng2018} separate calibration from
	recalibration model risk. \citet{alfeus2026} models JIBAR--ZARONIA spread
	dynamics around scheduled events, which is historical spread modelling rather
	than identification of a risk-neutral valuation.
	...
	The gap lies between these two literatures: a characterised set of transition
	models on one side, and a finite-sample analysis of the hedging losses those
	models imply on the other. Connecting them is the object of this research.
	...
	benchmark, admitting explicit covariance calculations and tractable
	simulation. The observation map from model to traded instrument prices is
	specified so that it prices actual cashflows before any identification claim
	is made.
	...
	Discounted terminal wealth comprises initial capital, cumulative trading gains
	and transaction costs; signed hedging loss is the discounted liability less
	that wealth. A sensitivity hedge and a mean--variance hedge provide the
	initial comparisons, with admissibility, financing, rebalancing and costs
	fixed beforehand, and are compared across compatible pricing models on common
	scenarios.
	...
	dependence, maturity, strike, observation noise, costs and regime changes, and
	including misspecified models. \textbf{Retrospective validation} withholds
	successor option quotes while fitting to legacy and curve information,
	revealing them only for evaluation; a historical LIBOR--SOFR dataset is the
	candidate. \textbf{The South African application} proceeds on the instruments
	and dates the data audit supports, reporting compatible valuation ranges,
	scenario sensitivity and observable hedge outcomes.

	Training, method selection, calibration and evaluation are separated
	chronologically, with gaps where the dependence argument requires them, so
	withheld data cannot enter calibration through model selection. Where licensed
	data cannot be obtained, the empirical design reduces to what the available
	observations support, with simulation carrying the controlled validation.

	% =============================================================
	\section{Contribution and timeline}

	The contribution has two linked parts: an identification analysis determining
	which successor valuations the legacy market fixes and how wide the remaining
	range is, and a finite-sample analysis of loss thresholds for implementable
	hedges under dependence and distributional change. Together these say what a
	converted option book can be valued at on the available evidence, and what can
	be guaranteed about the losses from hedging it.

	Months 1--6 cover foundations, the literature review and the data audit;
	7--14 the transition model and identification; 15--22 hedging and the
	statistical analysis; 23--29 simulation and empirical evaluation; and 30--36
	synthesis, revision and examination preparation. Month 6 is the first review
	point, confirming data access, the model class and the direction of the
	identification results.

	% =============================================================
	% References block exactly as in the MSc dissertation
	\clearpage
	\phantomsection
