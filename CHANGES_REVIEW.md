...
## 2026-09-19 - Chapter 1 drafted; rounds one closed for Chapters 6 and 7
- Chapter 1 written from the finished results rather than from the plan, at twenty-four pages against a twenty-four page budget. The two negative feasibility findings are stated as contributions in their own right: at a calendar year of daily observations the record cannot carry the block length the certificate bound prefers, and at the shortest block length the coupling term alone exceeds the level being certified.
- Chapter 6 round one closed, all twenty-seven items applied and none declined. Four acceptance criteria that arithmetic showed to be unfalsifiable were replaced: one was satisfied by any realised breach frequency in ninety-three of the one hundred and eighty cells of its own pre-registered grid, another could fail in only two, a third stated no decision rule although the results table printed its verdict, and a fourth had drifted to an estimator other than the one Chapter 5 pre-registered. The per-period hedging error, the number of deployment blocks and their separation are now fixed where they are used.
- One review demand was itself wrong and is corrected in the chapter: the difference between the naked midpoint and the naive mark carries a factor the review omitted, so the effect is six hundredths of a basis point rather than one and a fifth, which reconciles it with the figure Chapter 5 already prints.
- Chapter 7 round one closed, nineteen items applied, none declined, three deferred. Its headline claim did not survive: the marginal route of Barber and Pananjady, which needs no blocking, delivers a level on the same twenty-four monthly scores that clears Chapter 6's own acceptance criterion, so the chapter's finding is restated as one about the blocked route and the calibration-conditional guarantee alone. The record convention is fixed once and propagated, which turns the eighteen-month rows of the feasibility table infeasible and removes the coupling multiplier the chapter had been quoting.
- The two-sided feasibility criterion of Proposition 7.1 was shown to be false by a counterexample at the chapter's own parameters, feasibility not being monotone in the record length; the statement is now necessity plus attainment, with the sufficient direction proved and the exact threshold left in a gap box.
- The synchronisation script was rebuilding Appendix C from a template that predated the appendix, discarding its introduction, its section labels and the reproducibility protocol written since. It now copies the appendix as written. Sixteen dangling references in the compiled manuscript are gone and the build carries none.
- The manuscript compiles at three hundred and seventy-two pages with every chapter but the conclusions drafted.
...
## 2026-09-11 - Chapters 4 and 6
- Chapter 4 round one closed at exactly its forty-page budget. The comparison between the blocked and unblocked routes is redone on equal footing, which moves the crossover from about ninety-nine thousand observations to about two thousand two hundred and reverses the chapter's recommendation. The block length is now described as optimal for the bound it minimises rather than for the problem, and the absence of any lower bound for the blocked route is recorded as open.
- Chapter 6 drafted at about thirty-eight pages: the natural experiment, a protocol with explicit hiding rules, the identification and certificate tests with every acceptance criterion fixed in advance, the comparison against the bootstrap and the regulatory backtests, a falsification list, and threats to validity. No number is reported as computed.
- Chapter 6 establishes two limits by arithmetic alone: the transition record cannot carry the block length the certificate bound prefers, and at the shortest block length the certificate is vacuous before any drift is charged.
