...
During round one, agents write in the working directory `"~/Desktop/PhD Thesis/Claude Workspace/thesis/`; `sync_from_workdir.sh` copies into the repository layout. From round two the repository is the working directory and the old path is retired. Commit messages are plain and describe the change; no tool attribution of any kind appears in git history.
