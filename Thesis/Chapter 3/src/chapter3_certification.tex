...
The policy is \emph{fitted} on a fitting sample $\Dfit:=X_{1:N_0}$ through a measurable map $\pi:\mathcal X^{N_0}\to\Pi$; we write $\pi$ both for the map and for the random element $\pi(\Dfit)$. In the thesis $\pi$ is the time-changed SABR valuation of Chapter~\ref{ch2-preliminaries} with the convention strike shift and volatility scaling of Chapter~\ref{ch5-identifiability}, together with delta and vega hedges computed from it; the certificate does not use this structure and any measurable rule serves.
...
\]
and, using the exact conformal term available for continuous $P$, $\ge0.9231-0.0391-0.0127=0.8713$. The actual coverage in this example, which is computable because $P$ and $Q$ are known, is $\E F_Q(\hat q^*)$ up to the coupling error, and Figure~\ref{fig:example} compares it to the two bounds.
