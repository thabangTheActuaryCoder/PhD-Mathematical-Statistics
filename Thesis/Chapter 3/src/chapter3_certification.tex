...
\gap{Three items concerning Theorem~\ref{thm:cov} rest on material outside this chapter or diverge in wording from the ground-truth document. (1) The proof of Lemma~\ref{lem:berbee} given above takes as known the existence of jointly measurable density versions for a kernel between Polish spaces and the Borel isomorphism theorem; both are standard, and Appendix~\ref{app:proofs} is to supply references and the details. (2) \emph{Reading of Assumption~\ref{ass:mix} (adjudicated).} The coefficients $\beta(k)$ are those of the state process $(X_t)$ that generates both $\Dfit$ and the hedging errors (Section~\ref{sec:cert-scores}). This is the only reading under which the count $(n+1)\beta(b)$ is proved: Lemma~\ref{lem:blocks} couples $\Dfit$ as its first block, and the error process $(e_t(\pi))_t$ of the fitted policy is a function of $\Dfit$ and so does not inherit the coefficients of $(X_t)$. Under the literal wording of the assumption, mixing of the hedging-error process of an already-fixed policy, the hypothesis would have to be a conditional one (given $\Dfit$, uniform in the conditioning value), the fitting sample would not be coupled, and the count would be $n\beta(b)$; that alternative is recorded, not adopted. The wording of Assumption~\ref{ass:mix} is kept verbatim from the ground truth and the ground truth should be amended to name the state process. (3) \emph{Asymptotic content (resolved).} The approximation $b_{n,k}(\delta)\approx1-\alpha-\sqrt{2\alpha(1-\alpha)\log(1/\delta)/n}$ and the factor $2.3$ appear inside the ground-truth statement of the theorem. They are a large-$n$ approximation to a skewed law, not an inequality, and at $n=25$, $\alpha=0.1$, $\delta=0.05$ they err by $0.07$ below the exact quantile while the normal approximation with exact moments errs by $0.015$ above it (Section~\ref{sec:cert-example}). They have been moved out of the theorem statement into Remark~\ref{rem:betaapprox}; the inequality $b_{n,k}(\delta)\ge1-\alpha-\sqrt{\log(1/\delta)/(2n)}$ remains in the statement and is proved. The ground-truth statement should be amended the same way.}
...
The bound is that of \cite{bobkov2019}, restated with its moment conditions as Theorem~\ref{thm:prelim-bobkov} of Chapter~\ref{ch:prelim}. A finite $(2+\epsilon)$-th moment makes $J_1(P)$ finite, because Markov's inequality gives $F(x)(1-F(x))\le\E|T|^{2+\epsilon}\,|x|^{-(2+\epsilon)}$ for $|x|$ large and $|x|^{-(1+\epsilon/2)}$ is integrable at infinity; every score law considered in the thesis satisfies it.
...
Two remarks on the inputs. The block length $b$ is treated here as given. It trades off the number of blocks against the coupling cost: a larger $b$ makes $\beta(b)$ smaller and $n$ smaller, and Chapter~\ref{ch:sharpness} shows that within $\Bclass{r}$ the calibration-conditional deficit $\sqrt{b/N}+Nb^{-(r+1)}$, which comes from the Beta law in Theorem~\ref{thm:cov}(ii) and the coupling term, is minimised at $b^*\asymp N^{3/(2r+3)}$. The marginal form (i) has no $\sqrt{b/N}$ term; its only dependence on $b$ is through $(n+1)\beta(b)$ and the ceiling in $k$. Section~\ref{sec:cert-example} exhibits the trade-off numerically. The gap length is set equal to $b$ because Assumption~\ref{ass:mix} demands at least $b$ and any longer gap wastes record; a gap longer than $b$ can be used with the coefficient $\beta(\text{gap})$ in place of $\beta(b)$, and the algorithm is unchanged.
...
Let $X_t=\varphi X_{t-1}+\sigma\zeta_t$ with $|\varphi|<1$, $\zeta_t$ i.i.d.\ standard normal, started from the stationary law $\mathcal N(0,v)$, $v:=\sigma^2/(1-\varphi^2)$. The process is strictly stationary and Markov. Proposition~\ref{prop:prelim-ar1} of Chapter~\ref{ch:prelim} proves that it is geometrically $\beta$-mixing with $\beta(k)\le|\varphi|^k/\sqrt2$ for $\varphi^{2k}\le\tfrac12$; the general theory of mixing for linear processes is in \cite{doukhan1994}. The sharper bound below is proved here, by the same route, because the example needs the smallest number the route gives.
...
\[
\beta(k)\le\tfrac12\sqrt{-\log\big(1-\varphi^{2k}\big)}\;\le\;\tfrac12|\varphi|^k\big(1-\varphi^{2k}\big)^{-1/2}\;\le\;\frac{|\varphi|^k}{2\sqrt{1-\varphi^2}}\qquad(k\ge1).
\]
\end{lemma}

\begin{proof}
The $k$-step kernel from $x$ is $\mathsf P^k(x,\cdot)=\mathcal N\big(\varphi^kx,\,v(1-\varphi^{2k})\big)$, obtained by iterating the recursion. Pinsker's inequality, $\dTV(\nu_1,\nu_2)\le\sqrt{\mathrm{KL}(\nu_1\|\nu_2)/2}$ \cite[Lemma~2.5]{tsybakov2009}, and the closed form of the Kullback--Leibler divergence between normal laws,
\[
...
\]
The second inequality of the lemma is $-\log(1-y)\le y/(1-y)$ for $y\in(0,1)$; the third is $1-\varphi^{2k}\ge1-\varphi^2$ for $k\ge1$, and gives the $k$-uniform constant $C=1/(2\sqrt{1-\varphi^2})$ of the geometric class $\beta(k)\le C|\varphi|^k$.
\end{proof}
...
\datanote{Table~\ref{tab:example} at $b=5$ and $b=20$ is computed by the same formulas as the $b=10$ column: $v_5=5+2\sum_{h=1}^4(5-h)2^{-h}=5+2(2+0.75+0.25+0.0625)=11.125$, $\dK=2\Phi(0.25/(2\sqrt{11.125}))-1=2\Phi(0.03748)-1=0.0299$; $v_{20}=20+2\sum_{h=1}^{19}(20-h)2^{-h}=56.0000$ to four decimals, $\dK=2\Phi(1/(2\sqrt{56}))-1=2\Phi(0.06682)-1=0.0533$; the Beta quantiles are the roots of $\Prob(\mathrm{Bin}(50,u)\ge46)=0.05$, namely $u=0.8262$, and of $\Prob(\mathrm{Bin}(12,u)\ge12)=u^{12}=0.05$, namely $u=0.05^{1/12}=0.7791$. At $b=5$ the conditional level $0.4837$ is stated with confidence only $0.1686$, because $n\beta(b)=0.78$; the row is reported to show the failure, not as a usable certificate. Every number in the table and in the text above was recomputed independently on 2026-09-09 (binomial sums by bisection, the drift-slack expectation by quadrature against the $\Beta(24,2)$ density) and agrees to the digits shown. Three valid bounds on $\beta(10)$ at $\varphi=\tfrac12$ are in circulation and must not be ``corrected'' into one another: the first inequality of Lemma~\ref{lem:ar1} gives $4.883\times10^{-4}$ (used here); the $k$-uniform third inequality, $|\varphi|^k/(2\sqrt{1-\varphi^2})$, gives $5.638\times10^{-4}$ and is the constant used by the certificate code of Appendix~\ref{app:data}, whose test case also sets $\mu=0$ and therefore reports a marginal level $0.8853$ and a conditional level $0.8126$ with confidence $0.9359$; Proposition~\ref{prop:prelim-ar1} of Chapter~\ref{ch:prelim} gives $|\varphi|^k/\sqrt2=6.905\times10^{-4}$. The code is to be extended to the drift case and the tighter constant so that it regenerates this table before submission.}
...
\paragraph{Connection to Chapter~\ref{ch:sharpness}.} Three of these limitations are the subject of the next chapter. The constant $1$ on $\dK$ in Theorem~\ref{thm:cov} is shown to be sharp by an adversarial construction that moves the bottom-$\varepsilon$ mass of $P$ to a far point. The block length is optimised within $\Bclass{r}$ for the calibration-conditional deficit of Theorem~\ref{thm:cov}(ii), giving $b^*\asymp N^{3/(2r+3)}$ and a blocked deficit of order $N^{-r/(2r+3)}$; the same chapter shows that this blocked rate is not minimax, since at fixed confidence the unblocked sample quantile has calibration-conditional error of order $N^{-1/2}$ for every $r>1$, so that blocking buys the exact Beta law and a failure probability depending on $(C,r)$ alone, not rate. The doctoral statement of that chapter is a mixing-driven lower bound at high confidence over $\Bclass{r}$, for which a hidden-regime construction of the kind used in Proposition~\ref{prop:noest} is the route. The dependent-data version of Lemma~\ref{lem:bl} closes the gap left in Section~\ref{sec:cert-drift}. Theorem~\ref{thm:cov} is the tool; the sharpness, the design and the lower bound are the contribution.
...
\paragraph{Chapter notes.} The blocking-and-coupling argument for split conformal prediction under $\beta$-mixing, with Berbee's lemma and Lemma~4.1 of \cite{yu1994} as the tools, is due to \cite{oliveira2024}; Theorem~\ref{thm:cov}(i) is a variant of their marginal result with the fitting sample counted as a block and the deployment block gap-separated, and is not claimed as new. For marginal coverage the blocked route is dominated by \cite[Cor.~1--2]{barber2026}, whose deficit for the unblocked split-conformal threshold on stationary $\beta$-mixing data is $2\beta(\tau)+2\beta(\tau^*)$ plus a term linear in $\tau/n$, minimised over the free lags $\tau,\tau^*$, of order $N^{-r/(r+1)}$ over $\Bclass{r}$; their Theorem~2 shows this tight for split conformal up to a constant. \cite{barber2023} give a third route, through weighted exchangeability, that yields a total-variation penalty without blocking. Two further gapped or thinned constructions exist: \cite{zheng2024} split a Markov chain into $K$ thinned subsequences with a gap of the order of the mixing time, and \cite{allohibi2026} use a three-way split with buffer gaps under geometric $\beta$-mixing; neither optimises the block length or proves a lower bound, and \cite{allohibi2026} note that marginal validity follows from \cite{oliveira2024} and \cite{barber2026} without any buffer. The Beta law of calibration-conditional coverage is \cite{vovk2012}, and its use as an operational small-sample level under exchangeability is in \cite{zwart2025}. The mechanism behind the drift term, a realised coverage $F_Q\circ F_P^{-1}(U_{(k)})$ once the test score is decoupled from the calibration block, so that $\dK(P,Q)$ is the exact penalty on a fixed half-line, appears as Proposition~13 of \cite{ramos2026}, who name blocking after \cite{yu1994} as one way to decouple and who reach the Beta law under $\alpha$-mixing asymptotically through a Berry--Esseen bound (their Proposition~15); Theorem~\ref{thm:cov}(ii) reaches it non-asymptotically under $\beta$-mixing through the coupling, which is the difference between the two routes. What this chapter adds to that literature is confined to the following: the conditional statement (ii) with the disintegrated coefficient and the Markov term $\beta(b)/\delta'$, Remark~\ref{rem:jointtv} on why a joint total-variation bound cannot give it, the fixed-policy laws and the accounting of policy dependence as one $\beta(b)$, the drift bound of Proposition~\ref{prop:w1} with its counterexample, and the non-estimability construction of Proposition~\ref{prop:noest}. None of these is deep; together they make the certificate usable on a hedging record. The sharpness of the drift constant, the design of the block length and the lower bound over $\Bclass{r}$ are the contributions of Chapter~\ref{ch:sharpness}.
