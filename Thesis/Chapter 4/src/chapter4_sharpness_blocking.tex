...
\begin{quote}\itshape
The constant $1$ on the Kolmogorov drift term of the certificate of Chapter~\ref{ch:certification} is sharp, attained by a construction. The block length minimising the certified shortfall of the blocked route over $\Bclass{r}$ is computed exactly and is not the balance point. The blocked rate is not minimax: the unblocked sample quantile attains $N^{-1/2}$ at fixed confidence and, compared on equal terms, is the smaller bound beyond a few thousand periods. A mixing-driven lower bound $(N^r\delta)^{-1/(r+1)}$ at confidence $\delta$ over all threshold procedures is stated with construction and route and is not proved. The partially observed drift is a target; the dependent Wasserstein rate is specialised.
\end{quote}
...
\paragraph{Is the drift term sharp?} The certificate charges $\dK(P,Q)$ with coefficient $1$, and nothing in Chapter~\ref{ch:certification} shows that a cleverer argument could not charge $\tfrac12\dK$; in a value-at-risk setting the nearest published bound carries a coefficient $2$ on a total-variation drift term and its author shows the factor $2$ is not removable for that argument \citep[p.~3]{schmitt2026}. The worked example of Chapter~\ref{ch:certification} shows the charge loose by a factor of nearly three, and whether the looseness is in the bound or in the example is the question. Section~\ref{sec:sharp-thm8} settles it: the bound is exact and the example is favourable.

\paragraph{How should the block length be chosen?} The block length sets the number of blocks, $n\approx N/(2b)$, and hence the width of the Beta law of calibration-conditional coverage, and it sets the coupling term $(n+1)\beta(b)$, which falls as $b$ grows. Algorithm~\ref{alg:cert} takes $b$ as an input and says nothing about where it should come from. Section~\ref{sec:sharp-block} fixes a mixing class and minimises the bound on the expected shortfall implied by Theorem~\ref{thm:cov}(ii) exactly. The answer is not the balance point of the two terms, which costs a third more than the minimum at the parameters of the worked example.
...
\paragraph{What is minimax?} Over the class $\Bclass{r}=\{\beta(k)\le Ck^{-r}\}$, how small can the two-sided calibration-conditional coverage error be made at confidence $\delta$ by any threshold computed from the record? Section~\ref{sec:sharp-notminimax} answers it at fixed $\delta$, where the answer is $N^{-1/2}$ and the mixing exponent does not appear. Section~\ref{sec:sharp-lower} states the answer at high confidence, where it does appear and where the certificates a risk committee signs live: the claim is $(N^r\delta)^{-1/(r+1)}$, produced by a regime-renewal process whose long runs are rare and damaging. It is not proved here; it is stated with its construction, its route and a box listing what is missing, and checked against the rate \citet[Cor.~2]{barber2026} prove marginally.

\paragraph{Map.} Section~\ref{sec:sharp-thm8} proves the sharpness theorem and Section~\ref{sec:sharp-functional} defines the coverage-error functional the rest of the chapter optimises. Sections~\ref{sec:sharp-block} to~\ref{sec:sharp-w1} take the five parts of the blocking theorem in turn. Section~\ref{sec:sharp-positioning} positions them against the five nearest papers, Section~\ref{sec:sharp-example} illustrates numerically, and Section~\ref{sec:sharp-discussion} states which parts carry doctoral weight.

The five parts of the blocking theorem are stated separately below, as Theorems~\ref{thm:block-b}, \ref{thm:block-rate}, \ref{thm:block-lower}, \ref{thm:block-drift} and Proposition~\ref{prop:block-w1}, because their status differs: two are proved, two are targets with routes, and one is a specialisation of a published result. Read together they are one statement about the calibration-conditional coverage error over a mixing class.
...
\item (Attained in the limit.) With $Q_M$ as in Definition~\ref{def:sharp-transport}, and with the limit taken as $M\to\infty$ in part (c) because no $Q_M$ with $M<\infty$ attains the infimum,
...
The term $\varepsilon\Prob(T\ge M)$ is the adversary's cost of hiding the mass at a finite point: if the threshold exceeds $M$ the transported mass is covered after all. It is a defect of the construction and not of the bound, since it can be made arbitrarily small by taking $M$ large, and it cannot be removed altogether, because $\delta_{+\infty}$ is not a probability measure on $\R$ and any construction placing mass beyond every threshold at once leaves the space of laws on $\R$, and with it the hypothesis of Theorem~\ref{thm:cov}.

The term $\varepsilon\Prob(F_P(T)<\varepsilon)$ is the adversary's cost of the threshold falling below the transported mass in the first place: if $T<q_\varepsilon$, the mass on $(-\infty,q_\varepsilon]$ was not covered before the transport either, so moving it away costs nothing. For split conformal it is exponentially small in the number of blocks, because the threshold is a high order statistic and $F_P(\hat q)$ concentrates near $k/(n+1)$.
...
\]
against the Hoeffding bound $1.0\times10^{-18}$. With $M$ chosen so that $\Prob(\hat q\ge M)\le10^{-6}$, the realised coverage loss under $Q_M$ is $\varepsilon$ to six decimal places: the certificate's charge is exactly right for this $Q$.
\end{example}

\datanote{The closed forms printed in Example~\ref{ex:sharp-corr}, evaluated in double precision: $1.416\times10^{-30}+2.98\times10^{-33}=1.42\times10^{-30}$ and $\exp(-41.405)=1.04\times10^{-18}$. No data enter.}
...
\]
so the loss is $\varepsilon\,C_T(P)$, not $\varepsilon$. For split conformal $C_T(P)=k/(n+1)\approx1-\alpha$, so the mixture establishes only that the coefficient on $\dK$ is at least $1-\alpha$, leaving a tenth of the constant unaccounted for at $\alpha=0.1$. The difference is where the mass comes from: a fraction $1-C_T(P)$ of what the mixture moves was uncovered already and so worthless to the adversary, whereas the bottom mass was covered with probability essentially one. Choosing the transport to hit exactly the mass the threshold covers is what turns $1-\alpha$ into $1$.
\end{remark}

\begin{remark}[Adversarial, not Le Cam]\label{rem:sharp-lecam}
Theorem~\ref{thm:sharp} is not a minimax statement and its proof is not an application of Le Cam's two-point method \citep[\S2.3, pp.~81--83]{tsybakov2009}. The two-point method is needed when the adversary must choose a law that is simultaneously damaging and hard to distinguish from the truth on the available data, so that the statistician cannot adapt. Here the deployment law $Q$ is never observed: the certificate is issued before the deployment block is realised, and no amount of data about $P$ constrains $Q$ beyond the assumed bound on $\dK(P,Q)$. The relevant quantity is therefore an infimum over a ball, $\inf\{C_T(Q):\dK(P,Q)\le\varepsilon\}$, and Theorem~\ref{thm:sharp} evaluates it by exhibiting the minimiser. No testing problem arises here.
\end{remark}

\begin{remark}[What sharpness does and does not say]\label{rem:sharp-scope}
Theorem~\ref{thm:sharp} fixes $P$, fixes the procedure, and finds the worst $Q$ in a Kolmogorov ball. A bound that holds for every $Q$ at a given $\dK$ is therefore loose at benign ones, as the worked example of \S\ref{sec:cert-example} of Chapter~\ref{ch:certification} is, and the room for improvement lies in the class of admissible $Q$ and nowhere in the argument.
\end{remark}
...
Let $(S_t)_{t\ge1}$ be a strictly stationary process $\Prob$ on the calibration record with continuous marginal distribution function $F_P$, and let $S_{1:N}$ be the record. From here a threshold procedure in the sense of Definition~\ref{def:sharp-proc} is taken to be record-measurable, $T=T(S_{1:N})$, which is the case optimised over below. Its \emph{conditional coverage error} at level $\alpha$ is
\[
\mathcal E(T;\Prob):=\big|F_P(T)-(1-\alpha)\big| ,
\]
...
\[
\mathcal E_\delta(T;\Prob):=\inf\big\{\varepsilon\ge0:\Prob\big(\mathcal E(T;\Prob)>\varepsilon\big)\le\delta\big\},
\]
...
\[
\mathcal E^*_\delta(N,r):=\inf_T\ \sup_{\Prob\in\Bclass{r}(C)}\ \mathcal E_\delta(T;\Prob).
\]
The law is displayed where more than one is in play and written $\mathcal E(T)$, $\mathcal E_\delta(T)$ otherwise; the constant $C$ of the class is suppressed in $\mathcal E^*_\delta(N,r)$ throughout.
\end{definition}

Three features of the definition need comment.

\paragraph{Why conditional.} The quantity $F_P(T)$ is the coverage delivered by the particular threshold in hand, not averaged over records. It is what Theorem~\ref{thm:cov}(ii) and Proposition~\ref{prop:twosided}(b) bound, and what a risk manager needs: having computed this threshold from this record, how much of the deployment law does it cover? The marginal coverage $\E F_P(T)$ is a different object, it is bounded for split conformal under $\beta$-mixing by \citet[Cor.~2]{barber2026} without blocking, tightly for that procedure up to a factor $(1-\alpha)/4$ \citep[Thm.~2]{barber2026} but not over procedures, and its minimax analysis over all procedures runs into the degeneracy described next. Definition~\ref{def:sharp-error} therefore takes the conditional object as the thing to be optimised, and Section~\ref{sec:sharp-positioning} records that the marginal problem is left to the literature that has solved it.

\paragraph{Why two-sided.} If $\mathcal E$ were replaced by the one-sided deficit $(1-\alpha-F_P(T))^+$, the infimum over procedures would be zero for every $N$, $r$ and $\delta$: the threshold $T\equiv+\infty$ has deficit zero, covers everything, and is useless. A minimax statement over all threshold procedures must therefore either charge for over-coverage or impose an efficiency constraint, and charging for over-coverage is the established form; its precedents are recorded in Remark~\ref{rem:twosided} of Chapter~\ref{ch:certification}. Over-coverage is not a technical device. In the application of this thesis the threshold is a reserve in currency units per block, and a threshold that over-covers is a reserve that is too large: capital held against a loss that will not occur. Proposition~\ref{prop:twosided}(c) of that chapter is the upper bound on $\mathcal E$ that the blocked route delivers.

\paragraph{Why the drift and the deployment gap are excluded.} Definition~\ref{def:sharp-error} contains no drift term and no coupling cost for the deployment block. Both are charged additively in Chapter~\ref{ch:certification}, by Theorem~\ref{thm:cov}(ii) and Proposition~\ref{prop:twosided}(b). Excluding them is legitimate because Theorem~\ref{thm:sharp} has just shown the drift charge exact, so it does not interact with the optimisation, and because both routes pay a deployment gap: the blocked route $\beta(b)/\delta'$ for a gap of length $b$, the unblocked route $\beta(b_g)/\delta'$ for a gap of length $b_g$ before the deployment block. Every comparison of the two routes below is therefore calibration-side to calibration-side, with that coupling removed from both. What remains is the only part of the certificate the choice of procedure controls.

\begin{remark}[The functional at the parameters of Chapter~\ref{ch:certification}]\label{rem:sharp-func-numbers}
Of the two-sided error reported in \S\ref{sec:cert-example} of Chapter~\ref{ch:certification}, Definition~\ref{def:sharp-error} isolates the conformal part, $0.0856$ at $n=25$, $\alpha=0.1$, $\delta=0.05$, from the drift and Markov terms, because it is the part the design of the procedure can move.
\end{remark}

\section{The block length that minimises the certified shortfall}\label{sec:sharp-block}

\begin{theorem}[The block length that minimises the certified shortfall of the blocked route]\label{thm:block-b}
...
\]
so at the minimiser the coupling term is exactly $1/(2(r+1))$ times the concentration term. The same computation with the constants kept gives the minimiser of $c_1\sqrt{b/N}+c_2Nb^{-(r+1)}$ as $\big(2(r+1)c_2/c_1\big)^{2/(2r+3)}N^{3/(2r+3)}$, so at $c_1=1/\sqrt2$ and $c_2=C$, the constants of \eqref{eq:sharp-shortfall}, the minimiser is $\big(2\sqrt2\,C(r+1)\big)^{2/(2r+3)}N^{3/(2r+3)}$, equal to $26.4$ at $r=2$, $C=1$, $N=500$ against the $23.9$ of \eqref{eq:sharp-bstar}, and it moves with $C$. What is quoted throughout is the minimiser of the normalised $f$; only the exponent $3/(2r+3)$ and the ratio $b^*/b_{\mathrm{bal}}$ of part~(c) are free of the two constants, and $f$ dominates the right side of \eqref{eq:sharp-shortfall} only when $C\le1$, which is the value used in every numerical illustration below. Hence
\[
...
\]
for every $r>1$, as the display in Example~\ref{ex:sharp-bstar} verifies numerically. At the optimum the coupling term is $2(r+1)$ times \emph{smaller} than the concentration term, because it falls in $b$ at rate $r+1$ while the concentration term rises only as $\sqrt b$: the minimiser of a sum sits where the derivatives cancel, not where the terms agree.
\end{proof}
...
\]
The balance point is $b_{\mathrm{bal}}=500^{3/7}=14.345$ with $f(b_{\mathrm{bal}})=2\times500^{-2/7}=0.33876$. The minimiser exceeds the balance point by the factor $1.67$, and the balance point costs $33$ per cent more. Rounded to whole periods this is a block of $24$ and a gap of $24$, giving $n=10$ blocks.
\[
\begin{array}{lrrrr}
\toprule
r & 1.5 & 2 & 3 & 5\\
...
\end{array}
\]
The block length falls as the mixing exponent rises: a process that forgets faster needs a shorter gap, and the record can afford more blocks. The penalty for using the balance point rises with $r$, because the coupling term then falls more steeply and the optimum moves further from equality.
\end{example}
...
The function $f$ discards the constants $1/\sqrt2$ and $C$ and replaces $n+2$ by $N/(2b)$. Minimising the first inequality of \eqref{eq:sharp-shortfall} instead, that is $g(b):=1/(2\sqrt{n+2})+(n+1)Cb^{-r}$ with $n=\lfloor N/(2b)\rfloor$, at $C=1$, $r=2$, $N=500$ gives an integer minimiser $b=25$ with $g(25)=0.1619$, against $g(24)=0.1634$, $g(23)=0.1651$, $g(20)=0.1661$ and $g(10)=0.3562$. The continuous relaxation $b^*=23.9$ therefore lands within one period of the integer minimiser of the exact bound, the two disagreeing only through the floor in $n$. The practical rule is to evaluate $g$ on the integers near $b^*$ and take the best; the closed form \eqref{eq:sharp-bstar} says where to look and what the rate is.
\end{remark}
...
It is also not an optimality statement about the blocked route. Theorem~\ref{thm:block-b} minimises an upper bound, and no lower bound on $\E[(1-\alpha-F_P(\hat q))^+]$ for the blocked route is proved anywhere in this chapter, so $b^*$ is the block length that minimises the certificate and is not shown to be the best block length for the problem.
\end{remark}

\gap{No lower bound on the expected calibration-conditional shortfall of the blocked route is established. The optimality of $b^*$ in Theorem~\ref{thm:block-b} is therefore optimality of the bound \eqref{eq:sharp-shortfall} and not of the design: a matching statement would need the concentration term $1/(2\sqrt{n+2})$ to be shown unimprovable at fixed $b$, which the exact $\Beta(k,n+1-k)$ law makes plausible, together with a lower bound on the price of the coupling, for which no construction is offered here.}
...
\qquad\text{and, more sharply,}\qquad
\Var F_N(x)\;\le\;\frac{\tfrac14+2C\,S_r(C)}N,\quad
S_r(C):=\sum_{k\ge1}\min\Big\{\frac1{4C},\,k^{-r}\Big\}\le\zeta(r),
\]
where $\zeta$ is the Riemann zeta function; the same bounds hold for the left-limit version $F_N(x^-):=N^{-1}\sum_t\ind{S_t<x}$.
\end{lemma}
...
\]
the series converging because $r>1$. For the sharper form, a covariance of two indicators never exceeds $\tfrac14$ in absolute value, so each lag may be bounded by $\min\{\tfrac14,Ck^{-r}\}=C\min\{1/(4C),k^{-r}\}$, which replaces $\zeta(r)$ by $S_r(C)$; at $C=1$, $r=2$ the series falls from $\zeta(2)=1.6449$ to $S_2(1)=0.8949$, and the Chebyshev constant of \eqref{eq:sharp-cheb} below falls from $11.90$ to $9.03$ at $\delta=0.05$. The displayed bounds of Theorem~\ref{thm:block-rate} are written with $\zeta(r)$, which is the weaker and simpler form; every conclusion drawn from them is strengthened by the refinement. The left-limit version is identical by the last sentence of Lemma~\ref{lem:sharp-cov}.
\end{proof}

\begin{remark}[No blocking, and where the mixing class enters]\label{rem:sharp-var}
Lemma~\ref{lem:sharp-var} uses no coupling, no gaps and no blocks: the mixing class enters through one convergent series and the effective sample size is $N$ up to an additive constant, not $N/(2b)$. That is the whole reason the unblocked quantile beats the blocked one in rate.
\end{remark}
...
\end{equation}
\item (Consequence.) At fixed $\delta$ the minimax coverage error over $\Bclass{r}(C)$ is of order $N^{-1/2}$ for every $r>1$, attained without blocking; the blocked rate $N^{-r/(2r+3)}$ of Theorem~\ref{thm:block-b} is slower by the factor $\sqrt{b^*}$. Blocking buys the exact $\Beta(k,n+1-k)$ law and a certificate whose only dependence on $(C,r)$ is the failure probability $(n+1)\beta(b)$. It does not buy rate. The two routes are comparable in size only calibration-side to calibration-side: \eqref{eq:sharp-cheb} charges no coupling for the deployment block, so the blocked figures compared against it in Remark~\ref{rem:sharp-constants} carry $n\beta(b)$ and not the $(n+1)\beta(b)$ of \eqref{eq:sharp-shortfall}, the deployment gap being paid by both routes as \S\ref{sec:sharp-functional} records.
\end{enumerate}
\end{theorem}
...
The gap must be closed on equal terms. The blocked column below rests on the exact $\Beta$ law, while \eqref{eq:sharp-cheb} is Chebyshev, the bluntest inequality available for a bounded sum, whose dependence on the confidence level is $\delta^{-1/2}=4.47$ at $\delta=0.05$. Section~\ref{sec:sharp-lower} names the tool that belongs on the unblocked side, the Fuk--Nagaev inequality for bounded strongly mixing sequences, whose Gaussian-type term has the sub-Gaussian size $\sqrt{2\sigma_r^2\log(2/\delta)/N}$ with the variance proxy $\sigma^2_r:=\tfrac14+2C\zeta(r)$ of Lemma~\ref{lem:sharp-var}, that is $3.540$ and $5.11N^{-1/2}$ at $C=1$, $r=2$, $\delta=0.05$, while its polynomial term $(N^r\delta)^{-1/(r+1)}$ is $0.0058$ at $N=10^4$ and does not bind. Evaluating all three at a range of record lengths, at $C=1$, $r=2$, $\delta=0.05$, with the deployment coupling removed from the blocked route so that every entry is a calibration-side quantity:
\[
\begin{array}{lrrrrrr}
\toprule
N & 500 & 2500 & 10^4 & 10^5 & 4\times10^5 & 1.6\times10^6\\
\midrule
\text{blocked, }\min_bg_{\mathrm{cal}}(b) & 0.160 & 0.105 & 0.0721 & 0.0377 & 0.0254 & 0.0171\\
\text{unblocked, Chebyshev \eqref{eq:sharp-cheb}} & 0.532 & 0.238 & 0.119 & 0.0376 & 0.0188 & 0.0094\\
\text{unblocked, sub-Gaussian estimate} & 0.229 & 0.102 & 0.0511 & 0.0162 & 0.0081 & 0.0040\\
\bottomrule
\end{array}
\]
Against the Chebyshev bound the two cross near $N=10^5$ periods, some four hundred years of daily data. Against the sub-Gaussian estimate they cross at about $2.2\times10^3$ periods, some nine years of daily data, and the recommendation reverses: beyond a few thousand periods the unblocked sample quantile is the better bound, and the blocked route is the smaller of the two only on short records. The refinement of Lemma~\ref{lem:sharp-var} lowers the two unblocked constants to $9.03$ and $3.88$ and both crossings further, to about $2.8\times10^4$ and $6.9\times10^2$ periods, so the table as printed is the conservative comparison. Blocking is therefore rate-suboptimal, and constant-optimal only against the Chebyshev bound: on the present accounting the unblocked route is preferred on any record longer than a few thousand periods. What blocking buys is set out in Section~\ref{sec:sharp-discussion}, and it is not the size of the bound.
\end{remark}

\datanote{The blocked row of the table in Remark~\ref{rem:sharp-constants} was produced by evaluating the calibration-side form $g_{\mathrm{cal}}(b)=1/(2\sqrt{n+2})+nCb^{-r}$ of \eqref{eq:sharp-shortfall}, with $n=\lfloor N/(2b)\rfloor$, over all integers $2\le b\le N/2$ and taking the minimum; the minimising block lengths are $25$, $43$, $78$, $210$, $378$ and $689$ at the six record lengths, the full form $g(b)$ with $(n+1)\beta(b)$ gives $0.162$, $0.106$, $0.0722$, $0.0377$, $0.0254$ and $0.0171$ at minimisers $25$, $43$, $78$, $210$, $378$ and $692$, so the choice between the two forms moves no entry of the table by more than a unit in the third digit. The Chebyshev row is $\sqrt{(\tfrac12+4C\zeta(r))/(N\delta)}$ in closed form with $\zeta(2)=1.644934$. The third row is an estimate and not a proved bound: it is the sub-Gaussian size $\sqrt{2\sigma^2_r\log(2/\delta)/N}$ that an exponential inequality of the shape quoted in Section~\ref{sec:sharp-lower} would deliver with the variance proxy $\sigma^2_r=\tfrac14+2C\zeta(r)=3.540$, no such inequality being applied here with a verified constant. The two crossings, $2.17\times10^3$ and $9.95\times10^4$ periods, are located by scanning $N$; the refined values $6.9\times10^2$ and $2.79\times10^4$ use $S_2(1)=0.894934$ in place of $\zeta(2)$. No data enter.}

\begin{remark}[What a matching lower bound over $\Bclass{r}$ would have to look like]\label{rem:sharp-whatnext}
Theorem~\ref{thm:block-rate}(b) is proved on independent scores, which lie in every class, so the exponent $r$ is invisible in it; that is a property of the problem at fixed confidence and not a defect of the proof. A lower bound in which $r$ appears must be sought where the independent case is not the worst case, and part (a) says where that is: its bound degrades as $\delta^{-1/2}$, so a construction whose damage is rare and large, rather than common and small, can beat it at small $\delta$. Section~\ref{sec:sharp-lower} builds one.
\end{remark}
...
Fix $r>1$ and $C>0$. There are constants $c_r>0$ and $\kappa_r\in(0,1]$, depending on $r$ and $C$ only, such that, writing
\[
\delta^{\max}_N:=\kappa_r\,N^{-(r-1)/2}
\]
for the upper end of the confidence range in which the mixing term binds, for every $N$ there is a strictly stationary process in $\Bclass{r}(C)$ with continuous marginal distribution function for which:
\begin{enumerate}[nosep,label=(\alph*)]
\item (Sample quantile.) the unblocked sample quantile $T_N$ of Theorem~\ref{thm:block-rate} satisfies, for every $\delta\in[N^{-r},\delta^{\max}_N]$,
\[
\Prob\Big(\mathcal E(T_N)\ \ge\ c_r\,\big(N^r\delta\big)^{-1/(r+1)}\Big)\ \ge\ 2\delta;
\]
\item (All procedures.) the same holds for every threshold procedure $T=T(S_{1:N})$, so that
\[
\mathcal E^*_\delta(N,r)\ \ge\ c_r\,\big(N^r\delta\big)^{-1/(r+1)},\qquad \delta\in[N^{-r},\delta^{\max}_N].
\]
\end{enumerate}
...
\end{equation}
and the mixing term binds exactly when $\delta<N^{-(r-1)/2}$, which is why the range of (a) and (b) stops at $\delta^{\max}_N$: for $r>1$ no confidence level free of $N$ lies inside it. At $N=500$, $r=2$ the range is $[4\times10^{-6},\,0.045\kappa_2]$, which covers the confidences at which a certificate is written. The exceedance probability is written $2\delta$ rather than $\delta$ so that $\Prob(\mathcal E>\varepsilon)>\delta$ strictly for $\varepsilon$ below the stated level, which is what Definition~\ref{def:sharp-error} requires before $\mathcal E_\delta$ can be bounded below.
\end{theorem}
...
\begin{route}
\emph{Part (a), the sample quantile.} Fix $\delta\in[N^{-r},\delta^{\max}_N]$ and set $m:=\lceil(N/\delta)^{1/(r+1)}\rceil$. Let $A$ be the event that some interval of the renewal skeleton lies entirely inside $\{1,\dots,N\}$ and has length at least $m$. By the renewal computation of Remark~\ref{rem:sharp-tradeoff}, $\Prob(A)\ge2\delta$ once $m$ is taken with $Nm^{-(r+1)}\asymp2\delta$, which changes $m$ and hence $c_r$ by the constant factor $2^{-1/(r+1)}$ and delivers the strict exceedance the statement needs; the restriction $\delta\ge N^{-r}$ keeps $m\le N$, so the event is not empty. On $A$, condition on the position and label of the long interval. The remaining $N-m$ scores are those of a stationary regime-renewal process with the long interval excised, and $F_N$ restricted to them concentrates at scale $(N-m)^{-1/2}\ll m/N$ by Lemma~\ref{lem:sharp-var} applied to the excised record, because $m/N\asymp(N^r\delta)^{-1/(r+1)}$ exceeds $N^{-1/2}$ exactly when $\delta<N^{-(r-1)/2}$, which is the upper endpoint $\delta^{\max}_N$ of the stated range up to the constant $\kappa_r$; the range cannot be extended to an $N$-free $\delta_0$ when $r>1$, because $N^{-(r-1)/2}\to0$. Hence, on $A$ and off an event of small conditional probability, the displacement of Remark~\ref{rem:sharp-run} is not cancelled, and $\mathcal E(T_N)\ge c_r m/N$ with $c_r$ a fixed multiple of $\gamma_0$ times the density of $P$ near its $(1-\alpha)$-quantile. Choosing $P_1,P_2$ so that $F_2-F_P$ is bounded away from zero at $q_{1-\alpha}(P)$ fixes $c_r$.
...
\emph{The upper bound and the crossover.} For the matching upper bound, apply the Fuk--Nagaev inequality for bounded strongly mixing sequences \citep[Ch.~6]{rio2017} to the centred sums $\sum_t(\ind{S_t\le q}-F_P(q))$ at $q=q_{1-\alpha-\varepsilon}(P)$ and $q=q_{1-\alpha+\varepsilon}(P)$, exactly as Chebyshev was applied at those two points in the proof of Theorem~\ref{thm:block-rate}(a). The inequality has the shape of a Gaussian-type term $(1+N\varepsilon^2/(r\sigma^2))^{-r/2}$ plus a polynomial term $N^{-r}\varepsilon^{-(r+1)}$. Setting the polynomial term equal to $\delta$ reproduces $\varepsilon\asymp(N^r\delta)^{-1/(r+1)}$, matching the lower bound; setting the Gaussian-type term equal to $\delta$ gives $\varepsilon\asymp N^{-1/2}g_r(\delta)$ with $g_r$ trapped between $\sqrt{\log(1/\delta)}$, the sub-Gaussian value, and $\delta^{-1/r}$, the value the stated form of the inequality delivers. The two terms are equal when $(N^r\delta)^{-1/(r+1)}=N^{-1/2}$, that is when $\delta=N^{-(r-1)/2}$; the mixing term dominates for smaller $\delta$. At $N=500$, $r=2$ the threshold is $\delta=500^{-1/2}=0.045$, so every certificate written at the conventional confidence levels of $5$ per cent or better sits in the regime where the mixing exponent, and not the Gaussian scale, sets the achievable coverage error.
\end{route}

\begin{remark}[Consistency check against a published rate]\label{rem:sharp-bp-check}
A high-confidence lower bound should agree with what is known about the expected error, and this one does. The identity $\E X=\int_0^1q_{1-\delta}(X)\,d\delta$ turns a bound on quantiles into a bound on the mean, and integrating the claimed rate over the whole confidence level gives
\[
\int_0^1\big(N^r\delta\big)^{-1/(r+1)}\,d\delta
= N^{-r/(r+1)}\int_0^1\delta^{-1/(r+1)}\,d\delta
= \frac{r+1}r\,N^{-r/(r+1)} ,
\]
the integral converging because $1/(r+1)<1$. The rate $N^{-r/(r+1)}$ is exactly what \citet[Cor.~1, eq.~(6)]{barber2026} obtain for the marginal coverage deficit of unblocked split conformal over $\Bclass{r}$, by minimising $(\tau+L)/(n-L+1)+2\beta(\tau)$ over the free lag $\tau$, up to the score-training lag $L$, which balances at $\tau\asymp N^{1/(r+1)}$. One qualification must be attached. Theorem~\ref{thm:block-lower} asserts the rate only on $[N^{-r},\delta^{\max}_N]$, and over that range alone the integral is smaller: $\int_0^\delta\delta'^{-1/(r+1)}d\delta'=\frac{r+1}r\delta^{r/(r+1)}$ at $\delta=\delta^{\max}_N$ gives order $N^{-r/2}$, and the contribution of $[0,N^{-r}]$ is $O(N^{-r})$. What the agreement tests is therefore the shape of the rate function $(N^r\delta)^{-1/(r+1)}$, whose mean over the full confidence range is the published expected-error rate; it is not a deduction of either statement from the other, and no contradiction is possible in either direction, since $|\E F_P(T)-(1-\alpha)|\le\E\,\mathcal E(T)$ by Jensen's inequality. This is the strongest evidence available short of the proof.
\end{remark}
...
\gap{Not established in this thesis. (1) Lemma~\ref{lem:sharp-renewal-beta}: the constant $\kappa$ in $\beta(k)\le\kappa\Prob(R_t>k)$ is not computed. The regeneration epoch after time $t$ is measurable with respect to the past through the residual life, so the naive identification of the $\beta$-coefficient with the no-renewal probability is not immediate and may require coupling two stationary renewal processes, with a constant that has not been extracted. (2) Theorem~\ref{thm:block-lower}(a): the concentration of the excised record, and hence the claim that the displacement of Remark~\ref{rem:sharp-run} is not cancelled, is asserted at the level of orders; conditioning on the position and label of the long interval destroys stationarity of the remainder, so Lemma~\ref{lem:sharp-var} does not apply verbatim, and $c_r$ is not computed. (3) Theorem~\ref{thm:block-lower}(b): the two-point reduction over all procedures is a route only. It may lose a logarithmic factor, or hold only over the regime-renewal subclass of $\Bclass{r}$ rather than over the whole class; the step that must be supplied is that no record-measurable procedure can detect a run of length $m_\delta$ well enough to correct for it, given that the two $N$-sample laws are $\delta$-close. (4) The $\delta$-dependence $g_r$ in \eqref{eq:sharp-minimax} is open: it is trapped between $\sqrt{\log(1/\delta)}$ and $\delta^{-1/r}$ and its true order is not determined. (5) No verified locator is available for the Fuk--Nagaev inequality: the chapter locator \citep[Ch.~6]{rio2017} is the one recorded in the mathematical record of this thesis, no theorem or page number has been checked against the printed text, so the upper half of \eqref{eq:sharp-minimax} rests on an unverified citation as well as on an unwritten proof. The sub-Gaussian estimate of Remark~\ref{rem:sharp-constants}, on which the comparison of the two routes at moderate record lengths turns, has the same status: it is the size the Gaussian-type term of that inequality would have at confidence $\delta$, and \eqref{eq:sharp-cheb} remains the only proved bound for the unblocked route. (6) The confidence range: the bound is claimed only for $\delta\le\delta^{\max}_N=\kappa_rN^{-(r-1)/2}$, which shrinks with $N$ when $r>1$, and $\kappa_r$ is not exhibited. Whether the rate also holds for $\delta$ above $\delta^{\max}_N$, where the Gaussian scale of Theorem~\ref{thm:block-rate} dominates and the mixing exponent is invisible, is not determined; the mathematical record of this thesis states the range with an $N$-free upper endpoint, which the route above cannot service for $r>1$.}
...
The process, its parameters and the certificate at $b=10$ are those of \S\ref{sec:cert-example} of Chapter~\ref{ch:certification}. Table~\ref{tab:example-sweep} works the same certificate at three block lengths.
...
\item Compute the minimiser $b^*=(2(r+1))^{2/(2r+3)}N^{3/(2r+3)}$ of the certified shortfall from \eqref{eq:sharp-bstar}. Do not use the balance point $N^{3/(2r+3)}$, which costs between $27$ and $53$ per cent more over $r\in[1.5,5]$.
...
\item Compare the two routes calibration-side to calibration-side, that is $\min_bg_{\mathrm{cal}}(b)$ with $nCb^{-r}$ against the unblocked bound with no deployment coupling in either, and report both together with the deployment gap each route needs. Against the proved Chebyshev bound of \eqref{eq:sharp-cheb} the blocked bound is the smaller below about $10^5$ periods at $r=2$; against the sub-Gaussian size an exponential inequality would give, only below about $2.2\times10^3$. On any longer record the unblocked sample quantile is the route to use, and the blocked route is chosen for the exact $\Beta$ law and not for the size of its bound.
...
\datanote{The range ``between $27$ and $53$ per cent'' in step~2 is the last row of the table in Example~\ref{ex:sharp-bstar}: $f(b_{\mathrm{bal}})/f(b^*)$ takes the values $1.275$, $1.327$, $1.411$ and $1.525$ at $r=1.5,2,3,5$. The two crossovers in step~4 are those of the table in Remark~\ref{rem:sharp-constants} and hold at $C=1$, $r=2$, $\delta=0.05$; they move with all three, and the second rests on an estimate and not on a proved bound.}

\section{Discussion}\label{sec:sharp-discussion}

\paragraph{What was shown.} Three statements are proved in full, and Section~\ref{sec:sharp-positioning} lists them against the literature. The Kolmogorov drift charge of the certificate is exact, by the bottom-mass transport of Theorem~\ref{thm:sharp}, whose two correction terms are identified and one of which is exponentially small for split conformal. The block length of the blocked route is chosen exactly over a polynomial mixing class: Theorem~\ref{thm:block-b} minimises the bound of \eqref{eq:sharp-shortfall} on the expected calibration-conditional shortfall, gives the minimiser and its constant, and shows that the point where the two terms balance costs a third more. And the resulting rate is not minimax: Theorem~\ref{thm:block-rate} proves that the unblocked sample quantile attains $(N\delta)^{-1/2}$ over every class $\Bclass{r}$ with $r>1$, by a covariance bound on half-line indicators and Chebyshev's inequality at two quantiles, and that no procedure beats $N^{-1/2}$ at fixed confidence, by a two-point argument on independent scores, which lie in every class.

\paragraph{What blocking buys, stated once.} It does not buy rate. It buys three things, and each is used in Chapter~\ref{ch:certification}: the exact $\Beta(k,n+1-k)$ law of the calibration-conditional coverage, hence an exact quantile in place of a concentration inequality; a drift term charged on a half-line that the coupling has frozen, hence the constant $1$ that Theorem~\ref{thm:sharp} shows to be sharp; and a certificate whose only dependence on the assumed mixing class is the single term $(n+1)\beta(b)$, so that a reader who disputes the class can recompute the level in one multiplication. It does not buy the smaller bound either, except on short records. Remark~\ref{rem:sharp-constants} compares the two routes calibration-side to calibration-side and the blocked bound is the smaller only below about $2.2\times10^3$ periods at $r=2$; the crossing at $10^5$ periods against the Chebyshev bound is a property of that inequality's $\delta^{-1/2}$ and not of the two routes, and a practitioner with more than about nine years of daily data should report the unblocked bound as the operative one.

\paragraph{What is not shown.} Itemised in the four boxes rather than in prose: the mixing-driven lower bound of Theorem~\ref{thm:block-lower}, whose part~(a) needs the concentration of an excised record, whose part~(b) needs the two-point reduction over all procedures, and whose upper half rests on an inequality invoked at an unverified locator; the partially observed drift of Theorem~\ref{thm:block-drift}, with no part proved; the absolute constant of Proposition~\ref{prop:block-w1}; and any lower bound at all on the blocked route's own shortfall, without which $b^*$ is optimal for the certificate and not for the design. The constants of Theorem~\ref{thm:block-rate} also differ by a factor of $316$ at the parameters of the example, so only the exponent is matched.

\paragraph{Limitations of the whole approach.} Three are worth naming beyond the individual gaps. First, everything here is conditional on an assumed mixing class, and Proposition~\ref{prop:noest} of Chapter~\ref{ch:certification} shows that the class cannot be established distribution-free from any finite record; the optimal block length is optimal for the class one assumes, and a process outside it defeats the design as thoroughly as it defeats the certificate. Second, the functional of Definition~\ref{def:sharp-error} strips out the drift and the deployment coupling, legitimately, but the optimisation is then of a part and not of the whole: at $N=500$ the drift and Markov terms contribute $0.049$ of the $0.1345$ two-sided error of Proposition~\ref{prop:twosided}(c) of Chapter~\ref{ch:certification}, so a third of it lies outside the optimisation. Third, no illustration of $b^*$ on a genuinely polynomially mixing process has been run: the sweep of Section~\ref{sec:sharp-example} is on a geometrically mixing autoregression, which lies in every class, and the honest statement is that the design rule has been derived and not yet demonstrated.

\paragraph{What carries doctoral weight.} Theorem~\ref{thm:block-b} is elementary: one differentiation of a two-term function, whose value lies in the exactness of the constant and in the correction of the instinct to balance the two terms. Theorem~\ref{thm:sharp} is a short construction, the kind of result a referee expects beside a bound of the form of Theorem~\ref{thm:cov}; its absence would be a defect and its presence is not a contribution of scale. Proposition~\ref{prop:block-w1} is a specialisation and is claimed as one. Theorem~\ref{thm:block-drift} is a plan.

Two things do carry weight, and one of them is negative. The negative one is Theorem~\ref{thm:block-rate}. The literature on conformal prediction under dependence treats blocking as the route to a guarantee and does not ask what the guarantee costs relative to doing nothing. The answer is a factor $\sqrt{b}$ in effective sample size, and the blocked rate $N^{-r/(2r+3)}$ is therefore not minimax in any coverage formulation: marginally it is dominated by \citet[Cor.~2]{barber2026} without blocking, and conditionally by the ordinary sample quantile for every $r>1$. Establishing that, with both halves proved and with the crossover computed so that a practitioner knows which route to use at the record length in hand, is a real result about a design that is in use.

The positive one is Theorem~\ref{thm:block-lower}, and it is not proved. The work remaining has three parts: the regime-renewal process of Definition~\ref{def:sharp-renewal} must be shown to lie in $\Bclass{r}$ with an explicit constant, which is routine renewal theory; the displacement argument of Remark~\ref{rem:sharp-run} must be made a proof for the sample quantile, a concentration argument on a conditioned, non-stationary remainder; and the two-point reduction must be carried out over all threshold procedures, which is the hard part and the one that answers the question \citet[Sec.~4]{barber2026} leave open in its calibration-conditional form. Remark~\ref{rem:sharp-bp-check} is the one piece of external evidence available. If the reduction holds only over the regime-renewal subclass, or loses a logarithmic factor, the statement is weaker and still says that the mixing exponent, not the Gaussian scale, sets the achievable calibration-conditional coverage error at the confidence levels a certificate is written at. If it fails altogether, the chapter retains Theorems~\ref{thm:sharp}, \ref{thm:block-b} and \ref{thm:block-rate}. That is the floor, and it is stated as the floor.

\paragraph{Connection to the empirical chapters.} Chapter~\ref{ch:usd} runs the certificate on the \gls{libor}-to-\gls{sofr} record and Chapter~\ref{ch:zar} on the converted \gls{jibar} book, whose \gls{zaronia} record is short enough for the blocked route to be the smaller bound. Both fix the block length by the design rule of \S\ref{sec:sharp-example}, report both routes at the record length in hand, and report the two-sided error of Proposition~\ref{prop:twosided}(c) of Chapter~\ref{ch:certification} at the chosen block length.

\section{Chapter notes}\label{sec:sharp-notes}

Moving the extremal mass of a law to a far point to make a distributional-distance bound tight is standard in robust statistics; the observation that the mass must come from the \emph{bottom} of the calibration law, a mixture attaining only $1-\alpha$, is Remark~\ref{rem:sharp-mixture}.
