...
\end{equation}
We take \eqref{eq:prelim-pricing} as the definition of the pricing rule. Its justification, that an arbitrage-free market admits at least one such $\Q$ and that \eqref{eq:prelim-pricing} then gives the unique price of every attainable payoff, is the fundamental theorem of asset pricing; see \cite{filipovic2009} for a treatment in the term-structure setting and \cite{karatzas1991} for the stochastic-calculus background. Nothing in the thesis depends on completeness of the market, only on the existence of one pricing measure, which is fixed once and for all.
...
\begin{lemma}[Abstract Bayes formula]\label{lem:prelim-bayes}
...
Theorem~\ref{thm:prelim-numeraire} is the Geman--El Karoui--Rochet theorem; the presentation follows \cite{brigo2006} and \cite{filipovic2009}. Its content for us is that a payoff may be priced under whichever num\'eraire makes the computation simplest, and that the result does not depend on the choice.

\subsection{Forward measures and forward rates}

\begin{definition}[Forward measure]\label{def:prelim-forward-measure}
...
\begin{proposition}[Forward rates are forward-measure martingales]\label{prop:prelim-forward-mart}
...
Fix an accrual window $[T,T+\Delta]$ containing $m$ business days with overnight fixings $r_1,\dots,r_m$ of the ZARONIA rate and day-count fractions $\delta_1,\dots,\delta_m$ with $\sum_i\delta_i=\Delta$. Each $r_i$ is fixed at the start of day $i$ and is $\mathcal F_{t_i}$-measurable, where $t_i$ is the start of that day.
...
For $\Delta=0.25$ and $\kappa\in\{0.1,\,0.5,\,1,\,2,\,5\}$, formula \eqref{eq:prelim-cg-hw} gives $c_{g_\kappa}=0.3354,\,0.3438,\,0.3545,\,0.3762,\,0.4431$. For the mean-reversion speeds typical of calibrated one-factor models, $\kappa\le1$, the clock at $T+\Delta$ is within $0.021\Delta$ of $T+\Delta/3$, that is within about half a day for a three-month window; the linear profile is then a good approximation. For very fast mean reversion the clock moves towards $T+\Delta$ and the linear profile understates the in-window variance.
\end{example}
...
\end{equation}
a SABR model with time-dependent vol-of-vol, for which no constant-parameter closed form of Hagan type exists. The caplet is approximated by the Hagan--Lesniewski--Woodward effective parameters $(\rho_{\mathrm{eff}},\nu_{\mathrm{eff}})$ for time-dependent SABR. Relative to Lemma~\ref{lem:prelim-tc} the total vol-of-vol variance is
\begin{equation}\label{eq:prelim-novol-total}
\int_0^{\tau^*}\tilde\nu(u)^2\,du=\nu^2(T+\Delta)\qquad\text{rather than}\qquad\nu^2\tau^*=\nu^2\Big(T+\frac\Delta3\Big),
\end{equation}
so that, on the ground-truth accounting in which smile curvature is proportional to total vol-of-vol variance, Lemma~\ref{lem:prelim-tc} understates smile curvature by $\tfrac23\nu^2\Delta$, which is $2\Delta/(3T)$ of $\nu^2T$, about $17\%$ for $T=1$y and $\Delta=3$m.
\end{proposition}
...
Because $\tilde\nu$ is not constant, Lemma~\ref{lem:prelim-tc} does not apply and Theorem~\ref{thm:prelim-hagan} cannot be used with a single $\nu$. The time-dependent extension of the Hagan expansion, given in Appendix~B of \cite{hagan2002} and developed by Hagan, Lesniewski and Woodward, replaces $(\rho,\nu)$ by effective constants $(\rho_{\mathrm{eff}},\nu_{\mathrm{eff}})$ such that the constant-parameter formula with these constants reproduces the time-dependent smile to the order of the expansion. In the ground-truth accounting the effective vol-of-vol variance is identified with the total $\int\tilde\nu^2$, which gives $\nu_{\mathrm{eff}}^2\tau^*=\nu^2(T+\Delta)$, that is $\nu_{\mathrm{eff}}^2=\nu^2(T+\Delta)/\tau^*=1.154\,\nu^2$ in the example; the curvature $\mathsf c\propto\nu^2$ of Proposition~\ref{prop:prelim-lsc} is then understated by Lemma~\ref{lem:prelim-tc} in the stated proportion.
\end{proof}
...
\begin{proof}
Write $\tilde\alpha_u=\alpha_0(1+I_u)+O(\tilde\nu^2)$ with $I_u:=\int_0^u\tilde\nu(s)\,d\tilde Z_s$, the first-order expansion of the stochastic exponential. Then $\tilde F_{\tau^*}-F_0=\alpha_0\tilde W_{\tau^*}+\alpha_0\int_0^{\tau^*}I_u\,d\tilde W_u+O(\tilde\nu^2)$, and its third central moment to first order is $3\alpha_0^3\E[\tilde W_{\tau^*}^2\int_0^{\tau^*}I_ud\tilde W_u]$, all other terms being of higher order or zero by symmetry. By It\^o's formula $\tilde W_{\tau^*}^2=2\int_0^{\tau^*}\tilde W_ud\tilde W_u+\tau^*$, and by the isometry for the product of two stochastic integrals against the same Brownian motion, $\E[\tilde W_{\tau^*}^2\int I\,d\tilde W]=2\int_0^{\tau^*}\E[\tilde W_uI_u]\,du=2\int_0^{\tau^*}\int_0^u\rho\tilde\nu(s)\,ds\,du=2\rho\int_0^{\tau^*}\tilde\nu(s)(\tau^*-s)\,ds$, interchanging the order of integration. This gives $\kappa_3$. For $\Var(V)$, $\tilde\alpha_u^2=\alpha_0^2(1+2I_u)+O(\tilde\nu^2)$, so $V-\E V=2\alpha_0^2\int_0^{\tau^*}I_u\,du+O(\tilde\nu^2)=2\alpha_0^2\int_0^{\tau^*}\tilde\nu(s)(\tau^*-s)\,d\tilde Z_s+O(\tilde\nu^2)$ by the stochastic Fubini theorem, and the isometry gives the stated variance. For constant $\tilde\nu\equiv\nu$ the two integrals equal $\rho\nu(\tau^*)^2/2$ and $\nu^2(\tau^*)^3/3$; equating the time-dependent expressions to the constant-parameter ones with $(\rho_{\mathrm{eff}},\nu_{\mathrm{eff}})$ gives \eqref{eq:prelim-eff}. The skew of the smile is driven by $\kappa_3$ and its curvature by $\Var(V)$ at this order, which is how the effective parameters reproduce the smile.
\end{proof}

\gap{The step in Proposition~\ref{prop:prelim-novol} from total vol-of-vol variance $\int\tilde\nu^2du$ to smile curvature is not supported by the effective-parameter theory that the proposition invokes. By Proposition~\ref{prop:prelim-nueff} the effective vol-of-vol weights $\tilde\nu(u)^2$ by $(\tau^*-u)^2$: a volatility shock that arrives when little of the rate variance remains has little to act on. For the linear profile, $\tau^*-u=\tfrac\Delta3 g^3$ on the in-window part of the clock, so the excess of the non-decaying variant over Lemma~\ref{lem:prelim-tc} in $\int\tilde\nu^2(\tau^*-u)^2du$ is $\nu^2(\Delta/3)^3\big(\tfrac37-\tfrac13\big)=\nu^2\Delta^3\cdot\tfrac{2}{567}$, against a total of $\nu^2(\tau^*)^3/3$. For $T=1$y, $\Delta=3$m this makes $\nu_{\mathrm{eff}}^2$ larger by $0.013\%$, not by $15$--$17\%$; the two variants produce the same smile to well within the accuracy of the Hagan expansion. The ground-truth statement is kept as written because the ground truth is the source of record. What must be established is which of the two accountings the council intends: if total vol-of-vol variance, then the proposition is correct as stated but its conclusion does not describe the caplet smile; if effective vol-of-vol, then the ``$17\%$ understatement'' should be withdrawn and the choice of variant declared immaterial for pricing at this order. Either way the empirical chapters can use Lemma~\ref{lem:prelim-tc}. A lesson has been appended.}
...
\figplan{Normal SABR smiles for the worked example: $\sigma_N(K)$ against $K-f$ on $[-150,150]$ basis points for (a) Lemma~\ref{lem:prelim-tc} at clock $\tau^*=1.0833$, quoted at $T+\Delta$; (b) constant-parameter SABR run to $T+\Delta=1.25$ without decay; (c) Proposition~\ref{prop:prelim-novol} on the ground-truth accounting with $\nu_{\mathrm{eff}}=0.4297$; the three at-the-money values $56.62$, $60.94$ and $56.73$ bp marked.}{Smile of the compounded caplet in the worked example. Curve (a) is the time-changed SABR smile of Lemma~\ref{lem:prelim-tc}; curve (b) ignores the decay; curve (c) is the non-decaying vol-of-vol variant on the ground-truth accounting. The vertical gap between (a) and (b) is the $7\%$ run-off effect; the gap between (a) and (c) is the disputed curvature effect, invisible at this scale. The figure will make the point of the gap box: the variant choice is immaterial for pricing, the decay is not.}
...
Every result stated in this chapter, with two exceptions, has been proved in full from the definitions. The exceptions are Theorem~\ref{thm:prelim-hagan}, the Hagan approximation, for which a derivation sketch and a citation are given because the full singular-perturbation computation is not the thesis's concern and is treated as the market's quoting convention; and the uniform DKW inequality, Theorem~\ref{thm:prelim-dkw}, which is stated and cited but not used, the thesis needing only the pointwise Corollary~\ref{cor:prelim-dkw-cov} that it proves from the Beta law. The two results carried over from the ground-truth document are in different states. Lemma~\ref{lem:prelim-tc} is now proved, including the time-change lemmas it depends on, and the proof confirms the council's remark that the same-$g$ condition rather than a Dambis--Dubins--Schwarz argument is its substance. Proposition~\ref{prop:prelim-novol} is derived as stated, and the derivation exposes a step that the statement takes for granted: that smile curvature is proportional to total vol-of-vol variance. Proposition~\ref{prop:prelim-nueff} shows that the effective-parameter theory the proposition invokes weights vol-of-vol variance by the square of the remaining clock, under which the two variants of the model are indistinguishable at the accuracy of the Hagan expansion. The statement has been kept and the doubt boxed.
